--[[
    ...
    Fully compatible with Delta, Hydrogen, Fluxus, Arceus X, Codex, Vega X, Electron, KRNL
    Version: 4.0.0 | Stable Release
    Last updated: 2026
]]

-- ================================
-- EXECUTOR DETECTION & OPTIMIZATIONS
-- ================================
...
local function DetectExecutor()
    local executors = {
        ...
        {name = "ScriptWare", check = function() return script_context and script_context.executor == "ScriptWare" end}
    }
...
local function SafeCall(func, ...)
    ...
    if not ok then warn("Shadow Hub Error: " .. tostring(res)) end
    return ok, res
end

-- Notification (works on all executors)
...
-- ================================
-- RAYFIELD UI LOADER (FALLBACK SAFE)
-- ================================
local Rayfield = nil
...
if not LoadRayfield() then
    Notify("Shadow Hub", "Failed to load UI. Retrying...", 3)
    task.wait(2)
    if not LoadRayfield() then
        Notify("Shadow Hub", "Could not load UI. Please check your internet.", 5)
        return
    end
end
...
local function ClickButton(button)
    ...
    -- Method 4: Mouse simulation (legacy)
    if not success then
        pcall(function()
...
-- Find button by pattern (name, text, tooltip)
        ...
        if source then
            for _, v in ipairs(source:GetDescendants()) do
                if v:IsA("TextButton") or v:IsA("ImageButton") then
                    ...
                    local tooltip = (v.ToolTip or ""):lower()
                    if name:find(pattern) or text:find(pattern) or tooltip:find(pattern) then
                        return v
                    end
                end
            end
...
local function AutoCollect()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if hasTouch or nameLower:find("pickup") or nameLower:find("loot") or nameLower:find("coin") or nameLower:find("drop") then
...
local function UseLuckPotion()
    local potion = FindButton("luck") or FindButton("potion")
    if potion then
        ClickButton(potion)
        return true
    end
...
-- Teleport System (customizable zones)
local TeleportZones = {"Start", "Forest", "Cave", "Desert", "Volcano", "Ice", "Meadow", "Swamp", "Castle"}
...
Watermark.Size = UDim2.new(0, 340, 0, 25)
...
AddToggle(MainTab, "AutoPotion", "Auto Use Luck Potion", UseLuckPotion, 60)

-- ========== FARM TAB ==========
AddToggle(FarmTab, "FarmNearest", "Farm Nearest Slime", FarmNearestSlime, 0.3)
...
Notify("Shadow Hub", string.format("Successfully loaded!\nExecutor: %s\nAll features ready.", ExecutorName), 5)