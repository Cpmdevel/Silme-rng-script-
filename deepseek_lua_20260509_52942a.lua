--[[
    ...
    FULLY FIXED - ALL EXECUTORS SUPPORTED
...
if not LoadRayfield() then
    ...
    task.wait(2)
    if not LoadRayfield() then
        Notify("Shadow Hub", "Cannot load UI. Check internet.", 5)
        return
    end
end
...
local function AutoCollect()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if hasTouch or nameLow:find("pickup") or nameLow:find("loot") or nameLow:find("coin") then
...
local function AutoUpgrade()
    ...
    for _, btn in ipairs(game:GetService("CoreGui"):GetDescendants()) do
        if btn:IsA("TextButton") and btn.Visible and btn.Active then
            ...
            if txt:find("luck") or txt:find("damage") or txt:find("speed") then
                ClickButton(btn)
                task.wait(0.2)
            end
        end
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Slime RNG Automation",
...
AddToggle(MainTab, "AutoPotion", "Auto Luck Potion", UsePotion, 60)
...
Rayfield:CreateButton({
    ...
    Callback = function() Stats.Rolls = 0; Notify("RNG", "Counter reset", 2) end,
    Parent = RNGTab
})
Rayfield:CreateButton({
    Name = "Force Roll",
...
Rayfield:CreateToggle({
    ...
    Flag = "FlyMode",
    Callback = function(val)
        if val then ToggleFly() else if Flying then ToggleFly() end end
    end,
    Parent = PlayerTab
})
Rayfield:CreateToggle({
...
Notify("Shadow Hub", string.format("Fully fixed and loaded!\nExecutor: %s\nEnjoy Slime RNG automation.", ExecutorName), 5)